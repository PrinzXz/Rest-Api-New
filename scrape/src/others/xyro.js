    ...
    return new Promise((resolve, reject) => {
        fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
            ...
            .then(res => {
                ...
                data = []
                $('div[id="main"]').find('ul[id="citatenrijen"] > li').each(async function (index, element) {
...
async function otakudesugetepsddl(url) {
    return new Promise((resolve, reject) => {
    axios.get(url)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#venkonten > div.venser > div.venutama > div.download > ul:nth-child(2) > li').each(async function(a, b) {
            let dati = {
...
async function quotesAnime() {
    return new Promise((resolve, reject) => {
        ...
        .then(({ data }) => {
            const $ = cheerio.load(data)
            const hasil = []
            $('div.kotodama-list').each(async function(l, h) {
                hasil.push({
    ...
    axios.get(`https://nhentai.to/search?q=${query}`)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('body > div.container.index-container > div').each(async function(a, b) {
            result = {
            author: '©lui',
            status: 200,
            index: `${no++}`,
    ...
    axios.get(`https://play.google.com/store/search?q=${query}&c=apps`)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#yDmH0d > c-wiz.SSPGKf.glB9Ve > div > div > c-wiz > c-wiz > c-wiz > section > div > div > div > div').each(async function(a, b) {
            result = {
            status: 200,
            author: '@lui',            
...
async function sektekomiksearch(query) {
    return new Promise((resolve, reject) => {
    axios.get(`https://sektekomik.xyz/manga?search=${query}`)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#swiper-wrapper-32f18f5ed9677ea4 > div').each(async function(a, b) {
            result = {
            status: 200,
            author: '@lui',            
    ...
    axios.get(`https://nekopoi.care/search/${query}`)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#content > div.postsbody > div.result > ul > li').each(async function(a, b) {
            result = {
            status: 200,
            author: '@lui',            
    ...
    axios.get(`https://kusonime.com`)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#venkonten > div.vezone > div.venser > div > div.rseries > div.rapi > div.venz > ul > div').each(async function(a, b) {
            result = {
    ...
    axios.get(`https://nekopoi.care`)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#boxid > div').each(async function(a, b) {
            result = {
    ...
    axios.get(url)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#content > div.postsbody > div > div.arealinker > div.boxdownload > div').each(async function(a, b) {
            let dati = {
    ...
    axios.get(url)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#thumbnail-container > div').each(async function(a, b) {
    ...
    return new Promise((resolve, reject) => {
        axios.get(`https://mangatoon.mobi/en/search?word=${query}`)
            .then(({
                ...
                const $ = cheerio.load(data)
                const hasil = [];
                 $('#page-content > div.search-page > div > div.comics-result > div.recommended-wrap > div > div ').each(async function(a, b) {
                    result = {
                    status: 200,
                    author: author,
    ...
    return new Promise((resolve, reject) => {
        axios.get(`https://www.webtoons.com/id/search?keyword=${query}`)
            .then(({
                ...
                const $ = cheerio.load(data)
                const hasil = [];
                 $('#content > div.card_wrap.search._searchResult > ul > li ').each(async function(a, b) {
                    result = {
                    status: 200,
                    author: author,
...
async function otakudesu(query) {
    return new Promise((resolve, reject) => {
        axios.get(`https://otakudesu.video=/?s=${query}&post_type=anime`).then(({data}) => {
                const hasil = []
                const $ = cheerio.load(data)
                $('#venkonten > div > div.venser > div > div > ul > li').each(async function (a, b) {
                        result = {
                            status: 200,
                            author: author,
        ...
        axios.get('https://seegore.com/?s=' + query).then((dataa) => {
            ...
            axios
                ...
                .then(({
                    ...
                    const format = []
                    const thumb = []
                    $('#post-items > li > article > div.content > header > h2 > a').each(async function(a, b) {
                        link.push($(b).attr('href'))
                    })
                    $('#post-items > li > article > div.content > header > h2 > a').each(async function(c, d) {
                        jud = $(d).text()
                        judul.push(jud)
                    })
                    $('#post-items > li > article > div.content > header > div > div.bb-cat-links > a').each(async function(e, f) {
                        upl = $(f).text()
                        uploader.push(upl)
                    })
                    $('#post-items > li > article > div.post-thumbnail > a > div > img').each(async function(g, h) {
...
async function kusonimeinfo(url) {
    return new Promise((resolve, reject) => {
    axios.get(url)
    .then((res) => {
        const $ = cheerio.load(res.data)
    ...
    return new Promise((resolve, reject) => {
        ...
        fetch('https://twdown.net/download.php', { method: 'POST', body: params })
            ...
            .then(res => {
                const $ = cheerio.load(res);
                data = []
                $('div.container').find('tbody > tr > td').each(async function (index, element) {
...
async function otakudesuongoing() {
    return new Promise((resolve, reject) => {
        axios.get(`https://otakudesu.bid`).then(({data}) => {
                const hasil = []
                const $ = cheerio.load(data)
                $('#venkonten > div > div.venser > div.venutama > div > div.rapi > div > ul > li').each(async function (a, b) {
                        result = {
                            status: 200,
                            author: author,
    ...
    axios.get(`https://komikindo.id/?s=${query}`)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            const hasil = [];
            $('#content > div.postbody > section > div.film-list > div').each(async function(a, b) {
            let dati = {
      ...
      axios.get(url)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            $('#content > div.animeinfos > div.episodelist > ul > li').each(async function(a, b) {
            result = {
      ...
      axios.get(url)
        .then(({
           data
            ...
            const $ = cheerio.load(data)
            $('#venkonten > div.venser > div:nth-child(8) > ul > li').each(async function(a, b) {
            result = {
...
async function ssweb2(url, device = 'mobile') {
     return new Promise((resolve, reject) => {
          const base = 'https://www.screenshotmachine.com'
          const param = {
            url: url,
